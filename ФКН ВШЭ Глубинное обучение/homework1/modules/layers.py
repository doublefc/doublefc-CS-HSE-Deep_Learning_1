...
class Linear(Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.weight = np.random.uniform(-1, 1, (out_features, in_features)) 
        self.bias = np.random.uniform(-1, 1, out_features)  if bias else None
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        if self.bias is not None:
            ...
            return input.dot(self.weight.T)
    ...
    def update_grad_parameters(self, input: np.array, grad_output: np.array):
        ...
        if self.bias is not None:
            self.grad_bias = grad_output.T @ np.ones(input.shape[0]) 
...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        self.sqrt_var = np.power(self.running_var + self.eps, 0.5)
        self.inv_sqrt_var = 1/self.sqrt_var
        self.input_mean = input - np.repeat(self.running_mean[:, np.newaxis].T, input.shape[0], axis=0)
        self.norm_input = self.input_mean*self.inv_sqrt_var
        
        #self.weight = np.repeat(self.weight[:, np.newaxis].T, input.shape[0], axis=0)
        
        if self.bias is not None:
        #    self.bias = np.repeat(self.bias[:, np.newaxis].T, input.shape[0], axis=0)
    ...
    def update_grad_parameters(self, input: np.array, grad_output: np.array):
        ...
        self.grad_weight =   (grad_output * input).mean(axis=0) 
                                                     
        if self.bias is not None:
            self.grad_bias = grad_output.mean(axis=0)  
...
class Sequential(Module):
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        for module in self.modules:
            ...
            self.outputs.append(module(self.outputs[-1]))
    ...
    def compute_grad_input(self, input: np.array, grad_output: np.array) -> np.array:
        ...
        self.grads = [grad_output]
        for i in range(len(self.modules)-1, -1, -1):
            self.modules[i].update_grad_parameters(self.outputs[i], self.grads[-1])
            self.grads.append(self.modules[i].compute_grad_input(self.outputs[i], self.grads[-1]))
