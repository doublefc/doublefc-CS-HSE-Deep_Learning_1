...
def assert_almost_equal(x, y, debug_msg='{}'):
    ...
    assert x.shape == y.shape, debug_msg.format(f'wrong array shape, {x.shape}, {y.shape}')
    #assert np.allclose(x, y), debug_msg.format(f'wrong array value,{x}, {y}')
    ...
    for _ in range(outer_iters):
        ...
        for _ in range(inner_iters):
            ...
            for attr in param_attrs:
                if getattr(module1, attr) is not None:
