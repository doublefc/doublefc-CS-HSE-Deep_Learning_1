...
class TextDataset(Dataset):
    ...
    def __init__(self, data_file: str, train: bool = True, sp_model_prefix: str = None,
        ...
        """
        if not os.path.isfile(sp_model_prefix + '.model') or train:
        ...
        """
        texts = list(map(lambda x: x[:self.max_length] if len(x)>self.max_length else x, texts))
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        length = len(indices)
        indices = [self.bos_id]+indices+[self.eos_id]+(self.max_length-length-2)*[self.pad_id]
        """
        YOUR CODE HERE (⊃｡•́‿•̀｡)⊃━✿✿✿✿✿✿
        ...
        """
        return torch.tensor(indices, dtype=torch.int64), length+2
