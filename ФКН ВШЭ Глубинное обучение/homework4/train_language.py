...
def plot_losses(train_losses: List[float], val_losses: List[float]):
    ...
    valid_perplexities = torch.exp(torch.tensor(val_losses))
    

    axs[1].plot(range(1, len(train_perplexities) + 1), train_perplexities, label='train')
    axs[1].plot(range(1, len(valid_perplexities) + 1), valid_perplexities, label='val')
...
def train(model: LanguageModel, optimizer: torch.optim.Optimizer, scheduler: Optional[Any],
          train_loader: DataLoader, val_loader: DataLoader, num_epochs: int, num_examples=5, temp=1):
    """
